    ...
    struct F {
      static Complexd spharmGround(int l, int m, Complexd th, Complexd ph) {
        ...
        case 3:
          ...
          case -1:
            return 0.125 * sqrt(21 / M_PI) * ph.i * (5 * pow2(ph.r) - 1) *
                   th.conj();
          case 2:
            return 0.250 * sqrt(105 / M_2PI) * pow2(ph.i) * ph.r * pow2(th);
          case -2:
            return 0.250 * sqrt(105 / M_2PI) * pow2(ph.i) * ph.r *
                   pow2(th.conj());
          case 3:
      ...
      for (int m = -l; m <= l; ++m) {
        for (double th = 0; th < M_PI; th += M_PI / 8) {
          for (double ph = 0; ph < M_2PI; ph += M_2PI / 8) {
            ...
            if (!al::within(ms.r - tr.r, -eps, eps) ||
                !al::within(ms.i - tr.i, -eps, eps)) {
              printf("\nY(%d,%d,%g,%g):\n\ttr=(% g, % g)\n\tms=(% g, % g)\n", l,
                     m, th, ph, tr.r, tr.i, ms.r, ms.i);
              EXPECT_TRUE(false);
            }
          }
        }
