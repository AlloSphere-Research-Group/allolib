...
TEST_CASE ( "LBAP Allosphere")
{
    ...
    for (int i = 0; i < fpb; i++) {
        for (int chan = 0; chan < 54; chan ++) {
            if (chan == 30 || chan == 31) {
                REQUIRE(aeq((float) audioData.out(chan,i) , (float) cos(0.25f * M_PI) *(i + 0.5f)));
            } else {
                REQUIRE(aeq(audioData.out(chan,i),0.0f));
            }
        }
    ...
    pose.pos(-1,tan(2* M_PI *20.5/360.0), 0); // hard left between two rings (between speakers 16 and 45 middle ring and 0 and 11 on top ring)
    audioData.zeroOut();
    lbapPanner.renderBuffer(audioData, pose, samples, fpb);
    for (int i = 0; i < fpb; i++) {
        for (int chan = 0; chan < 54; chan ++) {
            if (chan == 16 || chan == 45 || chan == 0 || chan == 11) {
                REQUIRE(aeq((float) audioData.out(chan,i) , (float) 0.5 *(i + 0.5f)));
            } else {
                REQUIRE(aeq(audioData.out(chan,i),0.0f));
            }
        }
