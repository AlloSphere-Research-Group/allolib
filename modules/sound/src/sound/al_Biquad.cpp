...
BiQuad::BiQuad(BIQUADTYPE _type, double _sampleRate)
    : mType(_type), mSampleRate(_sampleRate), enabled(true) {
  mBD.x1 = mBD.x2 = 0;
  mBD.y1 = mBD.y2 = 0;
...
double BiQuad::operator()(double sample) {
  ...
  // 2X oversampling attempt... also need to double sampleRate for coefficient
  // computation in set() method
  //    sample = 0;
  //
  //    result = mBD.a0 * sample + mBD.a1 * mBD.x1 + mBD.a2 * mBD.x2 - mBD.a3 *
  //    mBD.y1 - mBD.a4 * mBD.y2;
  //
  //    // shift y1 to y2, result to y1
  //    mBD.y2 = mBD.y1;
  //    mBD.y1 = result;
