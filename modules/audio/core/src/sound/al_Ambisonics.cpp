...
AmbiBase::AmbiBase(int dim, int order) : mDim(dim), mOrder(0), mWeights(0) {
  this->order(order);
}

AmbiBase::~AmbiBase() { delete[] mWeights; }

void AmbiBase::dim(int dim) {
  assert(dim > 1 && dim < 4);
  if (dim != mDim) {
    mDim = dim;
...
AmbiDecode::AmbiDecode(int dim, int order, int numSpeakers, int flav)
    : AmbiBase(dim, order), mNumSpeakers(0), mDecodeMatrix(nullptr) {
...
void AmbiDecode::flavor(int type) {
  if (type < 4) {
    ...
    for (int i = 0; i < No; ++i)
      mWOrder[i] = flavorWeights[flavor()][i][order()];
    updateChanWeights();
  }
}

void AmbiDecode::numSpeakers(int num) { resizeArrays(channels(), num); }

// void AmbiDecode::zero(){ memset(mFrame, 0, channels()*sizeof(float)); }

void AmbiDecode::setSpeakerRadians(int index, int deviceChannel, float az,
                                   float el, float amp) {
  if (index >= numSpeakers()) {
    std::cout << "AmbiDecode::setSpeakerRadians() Warning. index out of range:"
              << index << std::endl;
    return;
  }
...
void AmbiDecode::print(std::ostream &stream) const {
  ...
  stream << "Decode Matrix:" << std::endl;
  int numHarm = orderToChannels(mDim, mOrder);
  // float *mat = mDecodeMatrix;
  for (int chan = 0; chan < mNumSpeakers; chan++) {
    stream << chan << "   :  ";
    for (int i = 0; i < numHarm; i++) {
      stream << decodeWeight(chan, i) << " ";
    }
    stream << std::endl;
  }
  //    stream << std::endl;
  stream << "Decode weights:" << std::endl;
  for (int i = 0; i < numHarm; i++) {
    stream << mWeights[i] << " ";
  }
  stream << std::endl;
...
AmbisonicsSpatializer::AmbisonicsSpatializer(const Speakers &sl, int dim, int order,
                                             int flavor)
    : Spatializer(sl), mDecoder(dim, order, sl.size(), flavor),
      mEncoder(dim, order){};

void AmbisonicsSpatializer::zeroAmbi() {
  assert(mAmbiDomainChannels.size() != 0 &&
         "Ambisonics not initiliazed! compile() not called.");
  memset(ambiChans(), 0, mAmbiDomainChannels.size() * sizeof(ambiChans()[0]));
}

void AmbisonicsSpatializer::configure(int dim, int order, int flavor) {
  mDecoder.dim(dim);
  mDecoder.order(order);
  mDecoder.flavor(flavor);

  mEncoder.dim(dim);
  mEncoder.order(order);
}
...
void AmbisonicsSpatializer::compile() {
  ...
  for (size_t i = 0; i < numSpeakers; i++) {
    mDecoder.setSpeakerRadians(i, mSpeakers[i].deviceChannel,
                               mSpeakers[i].azimuth, mSpeakers[i].elevation,
                               mSpeakers[i].gain);
  }
}
...
void AmbisonicsSpatializer::renderBuffer(AudioIOData &io, const Vec3f &pos,
                                         const float *samples,
...
void AmbisonicsSpatializer::renderSample(AudioIOData &io, const Vec3f &pos,
                                         const float &sample,
  ...
  //    // cheaper:
  //    Vec3d direction =
  //    mListener->quatHistory()[frameIndex].rotateTransposed(urel);
  //  Vec3d direction = listeningPose.vec();
