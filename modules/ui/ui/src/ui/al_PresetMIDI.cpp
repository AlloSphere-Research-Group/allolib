...
void PresetMIDI::connectNoteToPreset(int channel, float presetLow, int noteLow,
                                     float presetHigh, int noteHigh) {
  if (noteHigh == -1) {
  ...
  if (noteHigh - noteLow != presetHigh - presetLow) {
    std::cout
        << "PresetMIDI warning different ranges for preset and note ranges."
        << std::endl;
  }
  for (int num = noteLow; num <= noteHigh; ++num) {
    ...
    //			std::cout << channel << " " << num << " " << newBinding.presetIndex
    //<< std::endl;
  }
}

void PresetMIDI::connectProgramToPreset(int channel, float presetLow,
                                        int programLow, float presetHigh,
                                        int programHigh) {
  if (programHigh == -1) {
    presetHigh = presetLow;
  ...
  if (programHigh - programLow != presetHigh - presetLow) {
    std::cout
        << "PresetMIDI warning different ranges for preset and program ranges."
        << std::endl;
  }
...
void PresetMIDI::setMorphControl(int controlNumber, int channel, float min,
                                 float max) {
...
void PresetMIDI::onMIDIMessage(const MIDIMessage &m) {
  ...
  if (m.type() == MIDIByte::NOTE_ON && m.velocity() > 0) {
    for (NoteBinding binding : mNoteBindings) {
      ...
      //binding.noteNumber << " " << binding.presetIndex << std::endl; 				std::cout
      //<< (int) m.channel() << std::endl;
    ...
    if (m.controlNumber() == mMorphBinding.controlNumber &&
      ...
      mPresetHandler->setMorphTime(mMorphBinding.min +
                                   m.controlValue() *
                                       (mMorphBinding.max - mMorphBinding.min));
    }
  }
}
