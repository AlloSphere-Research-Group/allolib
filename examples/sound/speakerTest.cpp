...
struct MyApp: App {
  ...
  Parameter durSecs {"duration", "", 0.4f, "", 0.0, 1.0}; // duration of each burst
  ParameterBool autoChange {"autoChange"};
  Trigger next {"next"};
  ParameterInt curOutput {"Current", "", 0, ""}; // current speaker index playing
  ...
  void onCreate() override {
    ...
    next.registerChangeCallback([&](float value) {count = 0 ; int nextOutput = curOutput + 1;
