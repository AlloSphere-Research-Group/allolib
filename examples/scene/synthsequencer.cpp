...
void makeLine(SynthSequencer &s, float startTime, int number,
              float duration, float startFreq, float endFreq) {

    for (int i = 0; i < number; i++) {
        // Add notes to the sequencer. The add function returns a reference to a voice
        // so you can call functions you have defined to configure it.
        s.add<SineEnv>(startTime + i*duration, duration).mOsc.freq( startFreq + ((endFreq - startFreq) * i/ (float) number));
    }
