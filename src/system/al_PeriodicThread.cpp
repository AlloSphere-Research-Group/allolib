...
PeriodicThread::PeriodicThread(double periodSec)
    : mAutocorrect(0.1), mUserData(nullptr) {
  period(periodSec);
}
...
PeriodicThread::PeriodicThread(const PeriodicThread& o)
    : Thread(o),
      mPeriod(o.mPeriod),
      mTimeCurr(o.mTimeCurr),
      mTimePrev(o.mTimePrev),
      mWait(o.mWait),
      mTimeBehind(o.mTimeBehind),
...
bool PeriodicThread::running() const { return mRun; }
...
void PeriodicThread::start(std::function<void(void* data)> function,
                           void* userData) {
  if (mRun) {
    stop();
  }
