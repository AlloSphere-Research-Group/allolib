...
File::File(std::string path, std::string mode, bool open_)
	: mPath(path), mMode(mode), mContent(0), mSizeBytes(0),
	  mFP(0)
{
	if(open_) open();
}
...
File::File(const FilePath& path, std::string mode, bool open_)
	: mPath(path.filepath()), mMode(mode), mContent(0),
	  mSizeBytes(0), mFP(0)
{
	if(open_) open();
}
...
FileList SearchPaths::listAll() {
	...
	for (auto const& s : mSearchPaths) {
		...
		minFileSys::readDir(path, children);
		for (auto const& c : children) {
			fileList.add(FilePath{c, path});
		}
	}
	return fileList;
}
