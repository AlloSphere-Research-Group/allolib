...
void RenderManager::clearColor(float r, float g, float b, float a,
                               int drawbuffer) {
...
void RenderManager::setClearDepth(float d) { mClearDepth = d; }

void RenderManager::clearDepth() { glClearBufferfv(GL_DEPTH, 0, &mClearDepth); }
...
void RenderManager::clear(float r, float g, float b, float a, float d,
                          int drawbuffer) {
  clearColor(r, g, b, a, drawbuffer);
  clearDepth(d);
}

void RenderManager::modelMatrix(const Matrix4f& m) {
  mModelStack.set(m);
  mMatChanged = true;
}
void RenderManager::viewMatrix(const Matrix4f& m) {
  mViewStack.set(m);
  mMatChanged = true;
}
void RenderManager::projMatrix(const Matrix4f& m) {
  mProjStack.set(m);
  mMatChanged = true;
}
...
void RenderManager::scale(float s) { scale(s, s, s); }
...
ShaderProgram& RenderManager::shader() { return *mShaderPtr; }
...
void RenderManager::camera(Viewpoint const& v) {
  ...
  viewport(vp.l, vp.b, vp.w, vp.h,
           (mFBOID == 0) ? mWindowPtr->highres_factor() : 1);
  mMatChanged = true;
}
...
void RenderManager::camera(Viewpoint::SpecialType v, int x, int y, int w,
                           int h) {
  switch (v) {
    ...
    case Viewpoint::ORTHO_FOR_2D: {
      // 1. place eye so that bottom left is (0, 0), top right is (width,
      // height)
