...
void PresetMapper::readEntries(std::string path) {
  ...
  // entry.name().substr(entry.name().size() - 4) == ".txt") { 			std::cout <<
  // "Found map: " << entry.name();
  // 		}
  // 	}
  // }
  ...
  FileList map_list = filterInDir(path, [](const FilePath& f) {
    ...
    if (al::checkExtension(f, ".presetMap"))
      return true;
    else
      return false;
  });
...
bool PresetMapper::archive(std::string mapName, bool overwrite) {
  ...
  if (overwrite) {
    if (File::isDirectory(fullPath)) {
      if (!Dir::removeRecursively(fullPath)) {
        std::cout << "Error removing directory: " << fullPath
                  << " aborting preset map archiving." << std::endl;
        return false;
      }
    } else {
    ...
    if (!Dir::make(fullPath)) {
      std::cout << "Error creating preset map archive directory " << fullPath
                << std::endl;
      return false;
    }
  } else {
  ...
  for (auto const& preset : presetMap) {
    std::string presetFilename =
        mPresetHandler->getCurrentPath() + preset.second + ".preset";
    if (!File::copy(presetFilename, fullPath)) {
      std::cout << "Error copying preset " << presetFilename
                << " when archiving preset map." << std::endl;
      ok = false;
    }
  }
  if (!File::copy(mPresetHandler->buildMapPath(mapName),
                  fullPath + "default.presetMap")) {
    std::cout << "Error copying map: " << mapName
              << " when archiving preset map." << std::endl;
    ok = false;
  }
...
bool PresetMapper::load(std::string archiveName) {
  if (!(archiveName.size() > 18 &&
        archiveName.substr(archiveName.size() - 18) == ".presetMap_archive")) {
...
std::vector<std::string> PresetMapper::listAvailableMaps(bool showArchives) {
  ...
  while(mapperDir.read()) {
          ...
          if (info.type() == FileInfo::DIR && showArchives) {
                  ...
                  if ( (name.size() > 18 && name.substr(name.size() - 18) ==
  ".presetMap_archive")) { mapList.push_back(name);
                  }
          } else if (info.type() == FileInfo::REG && !showArchives) {
                  std::string name = info.name();
                  if ( (name.size() > 4 && name.substr(name.size() - 4) ==
  ".txt")
                       || (name.size() > 10 && name.substr(name.size() - 10) ==
  ".presetMap")) { mapList.push_back(name);
                  }
          }
  }
  */
...
bool PresetMapper::restore(std::string mapName, bool overwrite,
  ...
  bool ok = true;
  if (mapName.size() > 18 &&
      mapName.substr(mapName.size() - 18) ==
          ".presetMap_archive") {  // restore from archive
    std::cout << "Restoring archive " << mapName << std::endl;
    std::string mapNewName =
        mapName.substr(0, mapName.size() - sizeof("_archive") + 1);
    ...
    while (mapDirectory.read()) {
            ...
            if (entry.type() == FileInfo::REG) {
                    if (entry.name().substr(entry.name().size() - 7) ==
    ".preset") { if (overwrite && File::exists(mPresetHandler->getCurrentPath()
    + "/" + entry.name())) { File::remove(mPresetHandler->getCurrentPath() +
    entry.name());
                            }
                            if (!File::copy(mPresetHandler->getCurrentPath() +
    mapName + "/" + entry.name(), mPresetHandler->getCurrentPath() +
    entry.name())) { std::cout << "Error restoring preset " << entry.name() << "
    for " << mapName << std::endl; ok = false;
                            }
                    } else if (entry.name() == "default.presetMap") {

                            if (overwrite &&
    File::exists(mPresetHandler->getCurrentPath() + "/" + mapNewName)) {
                                    File::remove(mPresetHandler->getCurrentPath()
    + "/" + entry.name());
                            }

                            if (!File::copy(mPresetHandler->getCurrentPath() +
    "/" + mapName + "/default.presetMap", mPresetHandler->getCurrentPath() + "/"
    + mapNewName)) { std::cout << "Error restoring preset map " << mapNewName <<
    " for " << mapName << std::endl; ok = false;
                            }
                    } else {
                            std::cout << "PresetMapper::restore() invalid file:
    " << entry.name() << std::endl;
                    }
            }
    }
    */
    ...
    for (int i = 0; i < list.count(); i += 1) {
      ...
      if (name.substr(name.size() - 7) == ".preset") {
        if (overwrite &&
            File::exists(mPresetHandler->getCurrentPath() + "/" + name)) {
        ...
        if (!File::copy(mPresetHandler->getCurrentPath() + mapName + "/" + name,
          ...
          std::cout << "Error restoring preset " << name << " for " << mapName
                    << std::endl;
          ok = false;
        }
        ...
        if (!File::copy(mPresetHandler->getCurrentPath() + "/" + mapName +
                            "/default.presetMap",
                        mPresetHandler->getCurrentPath() + "/" + mapNewName)) {
          std::cout << "Error restoring preset map " << mapNewName << " for "
                    << mapName << std::endl;
          ok = false;
        }
      } else {
        std::cout << "PresetMapper::restore() invalid file: " << name
                  << std::endl;
      }
    }
...
void PresetMapper::findPresetMaps() {
  ...
  while (presetDir.read()) {
          FileInfo entry = presetDir.entry();
          if (entry.name().size() > 4 && entry.name().substr(entry.name().size()
  - 4) == ".txt") { std::cout << "Found map: " << entry.name() << std::endl;
          }
  }
  */
