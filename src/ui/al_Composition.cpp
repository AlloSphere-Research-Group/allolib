...
void Composition::stop() {
  ...
  mSequencer->stopSequence();  // Sequencer should also stop if in the middle of
                               // a sequence.
}

bool Composition::playArchive(std::string archiveName) {
  std::string compositionName = archiveName;
  if (compositionName.size() > 20 &&
      compositionName.substr(compositionName.size() - 20) ==
          ".composition_archive") {
    compositionName = compositionName.substr(0, compositionName.size() - 20);
  } else {
    archiveName += ".composition_archive";
  }
  std::cout << getRootPath() + archiveName << "--"
            << getRootPath() + archiveName + "/" + compositionName << std::endl;
  if (File::isDirectory(getRootPath() + archiveName) &&
      File::exists(getRootPath() + archiveName + "/" + compositionName +
                   ".composition")) {
    mPlayerLock.lock();
    stop();
...
bool Composition::restore(std::string compositionName, std::string path,
                          bool overwrite) {
...
void Composition::write() {
  ...
  if (!f.is_open()) {
    std::cout << "Error while opening composition file: " << fileName
              << std::endl;
    return;
  }
  f << fileText << "::" << std::endl;
  if (f.bad()) {
    std::cout << "Error while writing composition file: " << fileName
              << std::endl;
  }
  f.close();
}

void Composition::registerBeginCallback(
    std::function<void(Composition *, void *)> beginCallback, void *userData) {
  mBeginCallback = beginCallback;
  mBeginCallbackData = userData;
  mBeginCallbackEnabled = true;
}

void Composition::registerEndCallback(
    std::function<void(bool, Composition *, void *)> endCallback,
    void *userData) {
...
bool Composition::consumeMessage(osc::Message &m, std::string rootOSCPath) {
  if (m.addressPattern() == rootOSCPath + "/composition/playArchive" &&
      m.typeTags() == "s") {
    std::string val;
    m >> val;
    std::cout << "play composition archive" << val << std::endl;
    playArchive(val);
    return true;
  } else if (m.addressPattern() == rootOSCPath + "/composition" &&
             m.typeTags() == "s") {
    std::string val;
    m >> val;
    if (val == getName()) {
...
std::vector<al::CompositionStep> Composition::loadCompositionSteps(
    std::string compositionName) {
  std::vector<al::CompositionStep> steps;
  std::string fullName = getCurrentPath();

  if (fullName.size() > 12 &&
      !(fullName.substr(fullName.size() - 12) == ".composition")) {
...
void Composition::playbackThread(Composition *composition) {
  ...
  }
  {
    ...
    // FIXME we should remember previous state of callbackd for sequencer to
    // reset when composition is done
    // TODO this should be an optional parameter whether to disable sequence
    // callbacks
    composition->mSequencer->enableBeginCallback(false);
    composition->mSequencer->enableEndCallback(false);
    if (composition->mBeginCallbackEnabled &&
        composition->mBeginCallback != nullptr) {
  ...
  while (composition->mPlaying) {
    ...
    // auto duration =
    // //std::chrono::time_point_cast<std::chrono::nanoseconds>(step.deltaTime/1e9);
    targetTime += std::chrono::microseconds(
        (int)(step.deltaTime * 1e6 - (granularity * 1.5 * 1000.0)));
    while (std::chrono::high_resolution_clock::now() <
           targetTime) {  // Granularity to allow more responsive stopping of
    ...
    std::cout << "Composition step:" << step.sequenceName << ":"
              << step.deltaTime << std::endl;
  ...
  if (composition->mSequencer
          ->running()) {  // TODO There is a very small risk of a run condition
