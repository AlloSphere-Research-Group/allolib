...
public:
	...
	bool load(Font& font, const char * filename, int fontSize, bool antialias){
		...
		const int w = textureWidth(fontSize);
		const int h = textureHeight(fontSize);

		// prepare luminance array
		std::vector<uint8_t> arr;
		arr.resize(w * h);

		const int rowstride = w;
		uint8_t * optr = arr.data();
		...
		for(int i=0; i < ASCII_SIZE; i++) {
			...
			uint8_t * image = (uint8_t*)(optr + offset);

			// write glyph bitmap into texture:
			FT_Bitmap *bitmap = &glyph->bitmap;
			if(antialias) {
				for(int j=0; j < int(bitmap->rows); j++) {
					uint8_t *pix = image + j*rowstride;
					uint8_t *font_pix = bitmap->buffer + j*bitmap->width;
				...
				for(int j=0; j < int(bitmap->rows); j++) {
					uint8_t *pix = image + j*rowstride;
					uint8_t *font_pix = bitmap->buffer + j*bitmap->pitch;
					for(int k=0; k < int(bitmap->width); k++) {
		...
		// luminance texture
		font.mTex.resize(w, h, Texture::R8, Texture::RED, Texture::UBYTE);
		font.mTex.submit(arr.data());
		font.mTex.filter(GL_LINEAR);
		font.mTex.bind_temp();
		// make `texture` in glsl return (r, r, r, 1) so we can multiply tint to get color we wanted
		GLint swizzleMask[] = {GL_RED, GL_RED, GL_RED, GL_ONE};
		glTexParameteriv(GL_TEXTURE_2D, GL_TEXTURE_SWIZZLE_RGBA, swizzleMask);
		font.mTex.unbind_temp();
