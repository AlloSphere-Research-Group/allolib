...
void ShaderProgram::onCreate() {
  ...
  mShaderV = std::make_unique<Shader>(mVertSource, al::Shader::VERTEX);
  mShaderV->compile();
  mShaderV->printLog();
  attach(*mShaderV);

  mShaderF = std::make_unique<Shader>(mFragSource, al::Shader::FRAGMENT);
  mShaderF->compile();
  mShaderF->printLog();
  attach(*mShaderF);

  mShaderG = std::make_unique<Shader>(mGeomSource, al::Shader::GEOMETRY);
  bool bGeom = !mGeomSource.empty();
  if (bGeom) {
    mShaderG->compile();
    mShaderG->printLog();
    attach(*mShaderG);
