...
void AudioSceneObject::updateHistory() { mPosHistory(mPose.pos()); }
...
SoundSource::SoundSource(double nearClip, double farClip, AttenuationLaw law,
                         DopplerType dopplerType, double sampleRate,
                         double farBias, int delaySize)
    : DistAtten<double>(nearClip, farClip, law, farBias),
      mSound(delaySize),
      mUseAtten(true),
      mDopplerType(dopplerType),
      mUsePerSampleProcessing(false),
      mCachedIndex(0),
      mSampleRate(sampleRate),
      mSpeedOfSound(340),
      mFrameCounter(0) {
  // initialize the position history to be VERY FAR AWAY so that we don't deafen
  // ourselves...
...
int SoundSource::bufferSize(double samplerate, double speedOfSound,
                            double distance) {
...
AudioScene::~AudioScene() {
  for (Listeners::iterator it = mListeners.begin(); it != mListeners.end();
       ++it) {
    delete (*it);
  }
}

void AudioScene::addSource(SoundSource& src) { mSources.push_back(&src); }

void AudioScene::removeSource(SoundSource& src) { mSources.remove(&src); }
...
/*
        ...
        Having one delayline per soundsource (for doppler) is itself quite
   taxing. e.g., at 44100kHz sampling rate, a speed of sound of 343m/s and an
   audible distance of 50m implies a delay of at least 6428 samples (need to add
   blocksize to that too). The actual buffersize sets the effective doppler
   far-clip; beyond this it always uses max-delay size (no doppler) The
   head-size sets the effective doppler near-clip.
*/
    ...
    for (Sources::iterator it = mSources.begin(); it != mSources.end(); ++it) {
      ...
      if (mPerSampleProcessing) {  // audioscene per sample processing
        ...
        for (int i = 0; i < numFrames; ++i) {
          Pose relpos(src.pose().pos() - l.pose().pos(),
                      src.pose().quat() / l.pose().quat());
          spatializer->renderSample(io, relpos, src.getNextSample(l), i);
        }
      } else {  // more efficient, per buffer processing for audioscene (does
                // not work well with doppler)
        src.getBuffer(l, mBuffer.data(), numFrames);
        Pose relpos(src.pose().pos() - l.pose().pos(),
                    src.pose().quat() / l.pose().quat());
        //				std::cout << l.pose().x() << "," <<
        //l.pose().z() << " ---- "; 				std::cout << src.pos().x << "," <<
        //src.pos().z << " ----- "; 				std::cout << relpos.x << "," << relpos.z <<
        //std::endl;
...
struct AmbiSource{
        ...
        void operator()(double * outA, double * inT, int numT, double
dopScale=1, double distCoef=2, double ampOffset=0){

                // do distance coding
                for(int i=0; i<numT; ++i){
                        double dis = dist();				//
distance from source double amp = scl::clip(distToAmp(dis, distCoef) -
ampOffset, 1.f);	// computing amplitude of sound based on its distance
(1/d^2) doppler.delay(dis*dopScale); inT[i] = doppler(inT[i] * amp);
                }
        ...
        static double distToAmp(double d, double coef=2){
                ...
                // we want amp=1 when distance=0 and a smooth transition across
0, so
        ...
        synz::Delay doppler;
        OnePole<> dist, angH, angV;		// Low-pass filters on source
position AmbiEncode<double> enc;
};


        // block rate
        // (double * outA, double * inT, int numT, double dopScale=1, double
distCoef=2, double ampOffset=0) ambiH (io.aux(AMBI_NUM_SOURCES), io.aux(0),
io.numFrames(), 1./32); ambiO (io.aux(AMBI_NUM_SOURCES), io.aux(1),
io.numFrames(), 1./32, 2, 0.1); ambiZn(io.aux(AMBI_NUM_SOURCES), io.aux(2),
io.numFrames(), 1./32, 2, 0.1);
        ...
        for(int s=0; s < 16; ++s){
                ...
                for(unsigned int c=0; c<ambiDec.numChannels(); c++){
                        ...
                        for(unsigned int i=0; i<io.numFrames(); ++i) out[i] +=
*in++ * w * 8.f;
                }
        }
*/
