...
bool SpeakerTriple::loadVectors(const std::vector<Speaker> &spkrs) {
  ...
  if (s3 != -1) { // 3d Speaker layout
    ...
    mat.set(tempMat(0, 0), tempMat(0, 1), s1Vec[2], tempMat(1, 0),
            tempMat(1, 1), s2Vec[2], s3Vec[0], s3Vec[1], s3Vec[2]);
  }
  vec[0] = s1Vec;
  vec[1] = s2Vec;
  vec[2] = s3Vec;
    ...
    while (itA != triplets.end()) {
      ...
      for (int i = 0; i < 3; i++) {
        ...
        for (Speaker s : spkrs) {
          ...
          if (trip.speakerChan[spkIdx1] == s.deviceChannel ||
              trip.speakerChan[spkIdx2] == s.deviceChannel) {
            continue;
          }

          Vec3d spkVec(s.vec().x, s.vec().y, 0.f);

          if (angle(vec1, vec2) > angle(vec1, spkVec) &&
              angle(vec1, vec2) > angle(vec2, spkVec)) {
  ...
  while (crossCounter != 0) {
    ...
    while (it != triplets.end()) {
      ...
      while (it2 != triplets.end()) {
        ...
        if ((trip.s1 == trip2.s1) && (trip.s2 == trip2.s2) &&
            (trip.s3 == trip2.s3)) {
          ++it2;
          continue;
        }
        ...
        for (int i = 0; i < 3; i++) {
          for (int j = 0; j < 3; j++) {
            ...
            if (isCrossing(cr, li, lj, ln, lm) ||
                isCrossing(-cr, li, lj, ln, lm)) {
...
void Vbap::print(std::ostream &stream) {
  ...
  for (unsigned i = 0; i < mTriplets.size(); i++) {
    stream << "Group #" << i << ":" << mTriplets[i].s1Chan << ":"
           << mTriplets[i].s1Vec << "," << mTriplets[i].s2Chan << ":"
           << mTriplets[i].s2Vec;
    if (mIs3D) {
