...
void AudioApp::initAudio(double audioRate, int audioBlockSize, int audioOutputs,
                         int audioInputs, int device) {
  AudioDevice dev = AudioDevice(device);
  initAudio(dev, audioRate, audioBlockSize, audioOutputs, audioInputs, device);
}

void AudioApp::initAudio(AudioDevice& dev, double audioRate, int audioBlockSize,
                         int audioOutputs, int audioInputs, int device) {
  mAudioIO.init(AppAudioCB, this, audioBlockSize, audioRate, audioOutputs,
                audioInputs);
  if (device >= 0) {
...
bool AudioApp::usingAudio() const { return audioIO().callback == AppAudioCB; }
