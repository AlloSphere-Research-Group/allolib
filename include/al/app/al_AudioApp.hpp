...
class [[deprecated("Use AudioDomain instead")]] AudioApp {
 public:
  ...
  virtual void initAudio(double audioRate, int audioBlockSize, int audioOutputs,
                         int audioInputs, int device = -1);

  virtual void initAudio(AudioDevice & dev, double audioRate,
                         int audioBlockSize, int audioOutputs, int audioInputs,
                         int device = -1);
