...
public:
   ...
   * @param prepend Determines whether the sub-domain should run before or after this domain
   * @return the created domain.
   *
   * The domain specified by DomainType must inherit from SynchronousDomian.
   * It will trigger an assertion if not (will have undefined behavior for Release builds)
   * This operation is thread safe if the domain can pause itself, but it is blocking, so
   * there are no guarantees that it will not interefere with the timely running of the
   * domain.
   */
   ...
   * @param subDomain
   *
   * This operation is thread safe, but it might block causing drops or missed deadlines for the
   * parent domain. If this is a problem, the domain should be stopped prior to adding/removeing
   * sub-domains.
   */
...
//  void unlock() { mSubdomainLock.unlock(); }
  ...
  /**
   * @brief Return time delta with respect to previous processing pass for this domain
   * @return time delta
   *
   * The time delta may be set internally by the domain so that it is available whenever the domain ticks.
   * It might be 0 for domains that don't support this functionality or that are tied to strict hardware clocks
   * where this information is not necessary
   */
...
protected:
  ...
  /**
   * @brief initializeSubdomains should be called within the domain's initialization function
   * @param pre initialize prepended domains if true, otherwise appended domains.
...
protected:
  /**
   * @brief callStartCallbacks should be called by children of this class after the domain has been set up to start, before going into the blocking loop
   */
  ...
  /**
   * @brief callStopCallbacks should be called by children of this class on the stop request, before the domain has been stopped
   */
